...
#article_url='http://travel.nytimes.com/2013/03/24/travel/miami-my-way.html'
article_url='http://www.nytimes.com/2013/05/02/fashion/williamsburg.html'
#article_url='http://www.nytimes.com/2013/04/14/magazine/anthony-weiner-and-huma-abedins-post-scandal-playbook.html' #  URL of the article you want to get
comments=nytimes_comments(article_url)

output = file('comments_FINAL5.txt','a')

for comment in comments: #loop through the list
    line = comment['commentBody'].encode('ascii','replace') #print out the comment text
    output.write(line)
output.close()

# for comment in comments: #loop through the list
#     print comment['commentBody'].encode('ascii','replace') #print out the comment text
    
    
# for comment in comments[0]: #loop through the list
#     print comment.encode('ascii','replace') #print out all the itmes that are stored
